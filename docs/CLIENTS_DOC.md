...
El módulo de Clientes gestiona de forma integral la relación con cada cliente del ERP Norte Abanicos. Además de almacenar la ficha de datos, se integra con ventas, logística y tesorería para ofrecer una experiencia tipo CRUD completo que permita seguir pedidos, facturas, pagos, remitos y cuentas corrientes con sus retenciones.

### Objetivo operativo
- Administrar la base de clientes (datos, cuentas secundarias, canales).
- Visualizar trazabilidad de comprobantes: **Pedido → Factura → Pago → Remito**, incluyendo entregas parciales.
- Controlar cuentas corrientes (Cuenta 1 / Cuenta 2), registrar pagos por distintas cuentas de tesorería y aplicar retenciones (ARCA, RENTAS, ARBA, SIRCREB, Municipalidades, etc.).
- Integrar la información en una UX que simule un CRUD completo de seguimiento de actividad por cliente.

### Características Principales
- ✅ CRUD completo de clientes con importación masiva.
- ✅ Generación automática de códigos por tipo de cliente (MAY001, MIN001, PER001).
- ✅ Diferenciación Persona Física (inscripto o consumidor final) vs. Persona Jurídica (CUIT obligatorio).
- ✅ Registro de CUIT/CUIL/DNI desde base madre y posibilidad de asociar un CUIT secundario.
- ✅ Paginación y filtros avanzados.
- ✅ Soft delete para clientes con pedidos.
- ✅ Estadísticas de clientes.
- ✅ Control de acceso basado en roles.
- ✅ Validaciones robustas y trazabilidad con ventas / tesorería.
...
interface Client {
  ...
  primary_tax_id?: string;     // CUIT/CUIL/DNI principal
  secondary_tax_id?: string;   // CUIT secundario asociado (opcional)
  person_type: PersonType;     // Persona Física / Jurídica
  tax_condition?: TaxCondition;// Inscripto / Consumidor Final / Responsable Inscripto
  account_one_balance?: number;// Saldo Cuenta 1
  account_two_balance?: number;// Saldo Cuenta 2
}
```
...
enum SalesChannel {
  ...
  SISTEMA_MF = 'sistema_mf',                         // Sistema MF Computers
...
- **sistema_mf**: Clientes registrados en el Sistema MF Computers
...
### Nuevos conceptos funcionales

- **Importación desde base madre:** endpoint o job que ingeste clientes existentes por CUIT/CUIL/DNI (ver sección [Importación de datos](#🚚-importación-de-datos)).
- **Cuentas corrientes por cliente:** cada cliente mantiene Cuenta 1 y Cuenta 2 con saldos independientes y trazabilidad de movimientos (ver [Cuentas corrientes y retenciones](#💳-cuentas-corrientes-y-retenciones)).
- **Retenciones asociadas a pagos:** se registran según jurisdicción (ARCA, RENTAS, ARBA, SIRCREB, Municipal, etc.).
- **Comprobantes enlazados:** Pedido → Factura → Pago → Remito, con soporte de entregas parciales.
...
| sales_channel | Debe ser: 'woocommerce_minorista', 'woocommerce_mayorista', 'mercadolibre', 'sistema_mf', 'manual', 'otro' |
...
| person_type   | Requerido. Si 'persona_juridica', CUIT obligatorio                               |
| primary_tax_id| Requerido. Validar formato CUIT/CUIL/DNI                                          |
| secondary_tax_id | Opcional. Validar formato CUIT y que no coincida con primaria                  |
| tax_condition | Opcional. Si persona física → {inscripto, consumidor_final, monotributo}. Si jurídica → {responsable_inscripto, exento} |
| account_one_balance | Opcional, número ≥ 0                                                        |
| account_two_balance | Opcional, número ≥ 0                                                        |
...
| person_type   | Si cambia, validar coherencia con tax_condition y CUIT                            |
| primary_tax_id| Si cambia CUIT, verificar que no esté asignado a otro cliente activo              |
| secondary_tax_id | Si se agrega, validar formato y unicidad para ese cliente                      |
...
### Ejemplo 1: Crear Cliente Mayorista con cuentas corrientes
...
curl -X POST http://localhost:3000/api/clients \
  ...
  -d '{
    ...
    "account_one_balance": 0,
    "account_two_balance": 0
  }'
```
...
### Ejemplo 6: Registrar retención en aplicación de pago

```bash
curl -X POST http://localhost:3000/api/clients/26/retentions \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "payment_id": 450,
    "retention_type": "arba",
    "certificate_number": "ARB-2025-987",
    "amount": 12500.50,
    "applied_at": "2025-11-06T12:30:00.000Z"
  }'
```
